...
(function() {
  var closestSpan, textLengthBeforeElement;

  textLengthBeforeElement = function(el) {
    var length;
    length = 0;
  ...
  this.SelectableTextBlock = (function() {

    function SelectableTextBlock(element) {
      var _this = this;
      this.element = element;
    ...
    SelectableTextBlock.prototype.isSelected = function() {
      var index, range;
      range = document.getSelection().getRangeAt(0);
      if (range.commonAncestorContainer === this.element.get(0)) {
        return !range.collapsed;
      } else {
        index = $(range.commonAncestorContainer).parents().index(this.element);
        return !range.collapsed && index >= 0;
      }
    };

    SelectableTextBlock.prototype.getRange = function() {
    ...
    SelectableTextBlock.prototype.select = function(callback) {
      return this.element.bind("textselect", function(event) {
        return callback.call(this, event.range);
      });
    };

    SelectableTextBlock.prototype.getNodeForIndex = function(offset) {
      var rangeEnd;
      rangeEnd = 0;
      if (offset === 0) {
        return this.element.children().first();
      } else {
        return this.element.children().filter(function() {
          var rangeStart;
          rangeStart = rangeEnd;
    ...
    SelectableTextBlock.prototype.split = function(index) {
      var firstHalf, nodeOffset, secondHalf, text;
      firstHalf = this.getNodeForIndex(index).first();
    ...
    SelectableTextBlock.prototype.addAttribute = function(start, end, attributes) {
      var endSpan, startSpan;
      startSpan = this.split(start);
      endSpan = this.split(end);
      return startSpan.first().nextAll().not(endSpan.first().nextAll()).css(attributes);
    };
...
(function() {
  ...
  module = angular.module("Litterary", []);
  ...
  angular.module("Litterary").directive("litteraryEditor", [
    "NotesService", function(NotesService) {
      ...
      ScopeExtensions = {
        load: function() {
          var _this = this;
          return NotesService.draft().then(function(note) {
            return _this.note = note;
          });
        },
        save: function() {
          var _this = this;
          this.isSaving = true;
          return NotesService.update(this.note).then(function(note) {
            _this.lastSaved = new Date();
            return _this.isSaving = false;
          });
        },
        getLastSavedAtText: function() {
          var hours, minutes, seconds;
          if (this.lastSaved != null) {
            hours = this.lastSaved.getHours();
            minutes = this.lastSaved.getMinutes();
            seconds = this.lastSaved.getSeconds();
            if (minutes < 10) {
              minutes = "0" + minutes;
            }
            if (seconds < 10) {
      ...
      return function(scope, element, attributes) {
        ...
        return scope.load(attributes.noteId);
      };
    }
  ]);
        ...
        link: function(scope, el) {
          scope.$watch("citation", function() {
            if (scope.hasActiveHighlight()) {
              return scope.positionCommentForm(scope, el);
            }
          });
          scope.submitComment = function() {
            this.createHighlight(scope.citation);
            return this.closeRangeComment();
          };
          scope.cancelRangeComment = function() {
            return this.closeRangeComment();
          };
          return scope.closeRangeComment = function() {
            return this.blurActiveHighlight();
          };
        }
      };
    }
    ...
    "NotesService", "$compile", "$sce", function(NotesService, $compile, $sce) {
      ...
      ScopeExtensions = {
        ...
        highlight: function(start, end) {
          return this.range(start, end).addClass("highlight");
        },
        getActiveHighlight: function() {
          var ranges;
          if (this.body) {
            ranges = this.body.filter(function(i, range) {
              return range.hasClass("highlight-live");
            });
        ...
        focusActiveHighlight: function(start, end) {
          this.blurActiveHighlight();
          this.range(start, end).addClass("highlight-live");
        ...
        getDocumentHtml: function() {
          if (this.hasNote()) {
            return $sce.trustAsHtml(this.toHtml());
          } else {
      ...
      return {
        link: function(scope, element, attributes) {
          var noteId, selectableBlock;
          angular.extend(scope, ScopeExtensions);
          noteId = parseInt(attributes.noteId);
          scope.createHighlight = function(citation) {
            return NotesService.citation(noteId, citation).then(function(cit) {
              return scope.addCitation(cit);
            });
          };
          scope.getNote(noteId).then(function(note) {
            var citation, _i, _len, _ref, _results;
            _ref = note.citations;
            _results = [];
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              citation = _ref[_i];
              _results.push(scope.addCitation(citation));
            }
            return _results;
          });
          ...
          element.on("mouseover", "span", function(event) {
            ...
            return scope.$apply(function() {
              ...
              active = (function() {
                ...
                for (_i = 0, _len = citations.length; _i < _len; _i++) {
                  id = citations[_i];
                  _results.push(scope.getCitation(id));
                }
                return _results;
              })();
          ...
          return scope.positionCommentForm = function(childScope, element) {
            var position, span;
            span = this.getLiveHighlight();
            position = span.position();
            element.css({
              top: position.top - 173,
              left: position.left
            });
            return childScope.citation = scope.citation;
          };
        }
      };
    }
...
(function() {
  ...
  angular.module("Litterary").service("NotesService", [
    "$http", function($http) {
      ...
      return {
        draft: function() {
          return $http.get("/api/v1/notes/draft").then(function(res) {
            return res.data;
          });
        },
        show: function(noteId) {
          return $http.get("/api/v1/notes/" + noteId).then(function(res) {
            return res.data;
          });
        },
        update: function(model) {
          ...
          return $http.post("/api/v1/notes", data).then(function(res) {
            return res.data;
          });
        },
        citation: function(noteId, model) {
          var d;
          d = {
            citation: model
          };
          d.authenticity_token = getAuthenticityToken();
          return $http.post("/api/v1/notes/" + noteId + "/citations", d).then(function(res) {
            return res.data;
          });
        }
      };
