...
(function ($) {
    ...
    normalizedRange = function(){
      var r = {};
      var properties = "anchorNode anchorOffset focusNode focusOffset isCollapsed".split(" ");
      $.each(properties, function(){
        r[this] = winSel[this];
      });
      r.text = winSel.toString();
      return r;
    },
		...
		$me = $.event.special.textselect = {
			...
			handler: function (evt) {
				...
				if (selObj.str !== curText || selObj.el !== conElement) {
					...
					selObj = { str : curText, el : conElement, range: normalizedRange() };

					// Change event type to our custom event
					evt.type = 'textselect';

					// Fire the simulated event
					$.event.trigger(evt, [selObj.str, selObj.el, selObj.range]);
				}
			}
		};
})(jQuery);
...
jQuery.initialize = function(selector, callback) {
  jQuery(function(){
    ...
    items.length && callback.call(items);
  });
}
